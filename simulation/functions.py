...
def write_route_file(
    ...
    with open(rou_file_path, "w") as f:
        ...
        random_trips(f=f, net=net, seed=seed, season=season, demand=demand, day=day)
        f.write("""</routes>""")
...
def parse_full_output_xml(file_path, net_df):
    """
...
def parse_lane_noise_xml(file_path, net_df):
    """
...
def parse_edge_noise_xml(file_path, net_df):
    """
...
def parse_trip_output_xml(file_path):
    """
...
def parse_emissions_xml(file_path, net_df):
    """
...
def modify_config(scenario_folder, area_folder, config_folder, output_folder, net_file):
    """
...
def aggregate_outputs(net_path, full_output_folder):
    # Convert the output files of raw xml to gzipped csv
    net_df = parse_net_xml(net_path)
        ...
        with open(filename, "rb") as f_in:
            with gzip.open(f"{filename}.gz", "wb") as f_out:
                ...
                if "edge_noise" in filename:
                    edge_noise = parse_edge_noise_xml(f_out, net_df)
                    ...
                    trip_info = parse_trip_output_xml(f_out)
                    trip_info.to_csv(
                        f"{file_without_extension}.csv.gz",
                        compression="gzip",
                    )
                elif "emission" in filename:
                    emissions = parse_emissions_xml(f_out, net_df)
                    emissions.to_csv(
                        f"{file_without_extension}.csv.gz",
                        compression="zip",
                    )
